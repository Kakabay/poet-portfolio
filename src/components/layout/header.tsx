...
import LoginWindow from "../shared/login-window";
...
export const texture = "/images/texture.png";

export const textureStyles =
  "mix-blend-multiply opacity-50 size-full object-cover absolute top-0 left-0 right-0";

const Header = () => {
  const active = useLoginStore().active;
  ...
  return (
    <>
      <AnimatePresence>
        ...
        {active && (
          <WindowPopup setActive={setActive}>
            <LoginWindow />
          </WindowPopup>
        )}
      </AnimatePresence>
      ...
      <header className="pt-4 xl:pt-6 flex flex-col gap-4">
        <div className="xl:hidden relative p-4 container overflow-hidden h-20 w-[328px] shadow-bottom contaner">
          <BgTexture className="bg-[url('/images/header-mob-shape.svg')] header-mob-path" />
        ...
        <div className="xl:flex hidden relative z-50 items-end container justify-between">
            ...
            <div className="flex gap-2 text-ON_SURFACE_VAR font-medium leading-[115%] text-[14px]">
              {socials.map((item, i) => (
                <Link
                  to={item.link}
                  key={i}
                  className="flex gap-1 transition-all hover:text-PRIM"
                >
                  {item.icon}
          ...
          <button
            ...
            className="relative cursor-pointer shadow-bottom book-hover after:auth-btn-path"
          >
            <BgTexture className="bg-[url('/images/auth-btn-shape.svg')] auth-btn-path" />
            <div className="relative py-3 px-2 flex items-center gap-1">
        ...
        <div className="container hidden xl:block">
          <nav className="relative shadow-bottom h-14 z-50">
