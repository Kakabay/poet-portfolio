...
const RegisterForm = () => {
  ...
  const handleEmailNextStep = async () => {
    // Запускаем валидацию только для поля email
    const isEmailValid = await form.trigger("email"); // возвращает true, если email корректный

    if (isEmailValid) {
      setActive(true); // Делаем секцию неактивной
    }
  };
  return (
    <Form {...form}>
